...
import { Music, User, Sliders, CheckCircle, ChevronLeft } from 'lucide-react';
...
const SubmitPage: React.FC = () => {
  ...
  try {
    ...
    navigate(`/sent?eventCode=${eventCode}&nickname=${encodeURIComponent(nick)}&songTitle=${encodeURIComponent(title)}&requestId=${docRef.id}`);
  } catch (err) {
    console.error(err);
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[50vh] text-neon-pink font-black italic animate-pulse">CARICAMENTO PALCO...</div>;

  if (!eventCode || !eventData) return (
    <div className="p-8 text-center mt-20">
      <h2 className="text-3xl font-black text-pink-500 uppercase italic">Codice Invalido</h2>
      <p className="text-gray-400 mt-2 uppercase tracking-widest text-xs">Richiedi all'admin il codice corretto della serata.</p>
    </div>
  );
  ...
  return (
    <div className="max-w-md mx-auto p-4 sm:p-6 pb-20">
      <div className="mb-8">
        ...
        <h2 className="text-4xl font-black italic uppercase text-white tracking-tighter leading-none mb-2">Prenota la tua canzone</h2>
      ...
      <div className="bg-neon-gradient border border-gray-800 rounded-[2.5rem] p-8 shadow-2xl neon-border-blue">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-3 tracking-[0.2em] ml-1">Il tuo Nome d'arte</label>
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-black border border-gray-800 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all placeholder:text-gray-800 font-bold"
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-3 tracking-[0.2em] ml-1">Titolo Canzone</label>
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-black border border-gray-800 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all placeholder:text-gray-800 font-bold"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className={`w-full bg-pink-600 hover:bg-pink-500 text-white font-black uppercase italic tracking-[0.15em] py-5 rounded-3xl transition-all shadow-xl shadow-pink-900/40 transform active:scale-95 flex items-center justify-center gap-2 ${submitting ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
